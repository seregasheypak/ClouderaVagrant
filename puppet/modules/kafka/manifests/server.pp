...
class kafka::server(
    ...
    $log_retention_minutes           = $kafka::defaults::log_retention_minutes,
    ...
    $queued_max_requests             = $kafka::defaults::queued_max_requests,
    $controlled_shutdown_enable      = $kafka::defaults::controlled_shutdown_enable,
...
) inherits kafka::defaults
{
    ...
    $kafka_default_conf = '/etc/default/kafka'
    $kafka_conf_dir = '/opt/kafka/config'
    file { $kafka_default_conf :
    ...
    $kafka_log4j_conf = "$kafka_conf_dir/log4j.properties"
    file { $kafka_log4j_conf:
