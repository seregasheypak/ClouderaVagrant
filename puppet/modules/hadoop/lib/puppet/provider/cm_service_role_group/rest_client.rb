...
Puppet::Type.type(:cm_service_role_group).provide(:rest_client) do
	...
	def make_url
		...
		base_url = "http://#{scm_host_port}/api/v4/clusters/#{scm_cluster}/services/#{scm_service}/roleConfigGroups"
		response = RestClient.get base_url
		role_group_display_name = (@resource[:scm_role_group])
		scm_role_group = "UNKNOWN"
		JSON.parse(response)["items"].each do |role_group|
			if (role_group_display_name.eql? role_group["displayName"])
				scm_role_group = role_group["name"]
			end
		end

		url = "http://#{scm_host_port}/api/v4/clusters/#{scm_cluster}/services/#{scm_service}/roleConfigGroups/#{scm_role_group}/config"
		return url
	end
	...
	def set_config(uri, config_list)
		unless config_list.empty?
			self.debug "uri is #{uri}"
			self.debug "items #{config_list}"
